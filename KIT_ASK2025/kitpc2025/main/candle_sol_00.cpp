...
ld intersection(const Polygon& P1, const Polygon& P2) {
	return area(sutherland_hodgman(P1, P2));
}
struct Seg {
	Pos s, e;
...
std::vector<Ve> VE[1 << 3][1 << 3];
std::vector<Vld> VX[1 << 3][1 << 3];
Pos cen;
Segs Z[1 << 3];
...
void clean(const Pos& s, const Pos& e, const Seg& s0, const int& c1, Ve& tmp, Ve& ve) {
	if (tmp.empty()) {
		if (inner_check_concave(H[c1], s0.p(.5), s, e)) {
			ve.push_back({ 0, 1 });
			ve.push_back({ 1, -1 });
		}
		return;
	...
	int szt = tmp.size();
	for (int v = 0; v < szt; v++) ve.push_back(tmp[v]);
	return;
}
void query(const int& q) {
	...
	//for (int i = 0; i < 8; i++) for (int j = 0; j < 8; j++) VE[i][j].clear(), VX[i][j].clear();
	...
	for (int i = 0; i < 3; i++) {//get events
		...
		if (!I[c1] && !I[c2]) {
			for (int _ = 0; _ < 2; _++) {
				VE[c1][c2].clear();
				VX[c1][c2].clear();
				int sz1 = H[c1].size(), sz2 = H[c2].size();
				VE[c1][c2].resize(sz1);
				VX[c1][c2].resize(sz1);
				for (int j = 0; j < sz1; j++) {
					Ve tmp;
					int j1 = (j + 1) % sz1;
					const Pos& s = H[c1][j], & e = H[c1][j1];
					Seg s0 = Seg(s, e);
					for (int k = 0; k < sz2; k++) {
						int k0 = (k - 1 + sz2) % sz2;
						int k2 = (k + 1) % sz2;
						const Pos& p0 = H[c2][k0], & p1 = H[c2][k], & p2 = H[c2][k2];
						cut(s, e, p0, p1, p2, VX[c1][c2][j], tmp);
					}
					clean(s, e, s0, c2, tmp, VE[c1][c2][j]);
					std::sort(VE[c1][c2][j].begin(), VE[c1][c2][j].end());
					std::sort(VX[c1][c2][j].begin(), VX[c1][c2][j].end());
				}
				std::swap(c1, c2);
			}
		}
	...
	if (!I[RED] && !I[GREEN] && !I[BLUE]) {//R & G & B
		...
		for (int i = 0; i < 3; i++) {
			...
			int sz = H[c0].size();
			for (int j = 0; j < sz; j++) {
				Ve ve(VE[c0][c1][j].size() + VE[c0][c2][j].size());
				std::merge(
					VE[c0][c1][j].begin(), VE[c0][c1][j].end(),
					VE[c0][c2][j].begin(), VE[c0][c2][j].end(),
					ve.begin()
				);
				ve.push_back({ 1, -1 });
				...
				ve.push_back({ 0, 1 });
				std::reverse(ve.begin(), ve.end());
				Vld V(VX[c0][c1][j].size() + VX[c0][c2][j].size());
				std::merge(
					VX[c0][c1][j].begin(), VX[c0][c1][j].end(),
					VX[c0][c2][j].begin(), VX[c0][c2][j].end(),
					V.begin()
				);
				V.push_back(1);
				...
				Seg s0 = Seg(s, e);
				//for (const Event& e : VE[c0][c1][j]) ve.push_back(e);
				//for (const Event& e : VE[c0][c2][j]) ve.push_back(e);
				//for (const ld& x : VX[c0][c1][j]) V.push_back(x);
				//for (const ld& x : VX[c0][c2][j]) V.push_back(x);
		...
		if (!I[c1] && !I[c2]) {
			...
			for (const int& ca : { c1, c2 }) {
				...
				for (int j = 0; j < sza; j++) {
					...
					Seg s0 = Seg(s, e);
					for (const Event& e : VE[ca][cb][j]) ve.push_back(e);
					for (const ld& x : VX[ca][cb][j]) V.push_back(x);
					ve.push_back({ 1, -1 });
					V.push_back(1);
					V.erase(unique(V.begin(), V.end()), V.end());
					//std::sort(ve.begin(), ve.end());
...
void solve() {
	...
	//freopen("../tests/candle/in/81.in", "r", stdin);
	//freopen("../tests/candle/ans00/81.txt", "w", stdout);
	std::cin >> N;
	P[0].resize(N);
...
//		std::string Qout = Dir + "ans00/" + (i < 10 ? "0" : "") + std::to_string(i) + ".out";
...
0.0000000037253
46666.6666666716337
1856464.4911134652793
2367600.9716966450214
1577663.4586694762111
22345203.7451870739460
20099.9999999925494
