...
struct TimeGuard {
	...
	TimeGuard(const char* tag_, std::chrono::duration<Rep, Period> limit)
		: tag(tag_),
		limit_ns(std::chrono::duration_cast<std::chrono::nanoseconds>(limit).count()),
		st(clk::now()) {
	}

	~TimeGuard() {
		const auto elapsed_ns =
			std::chrono::duration_cast<std::chrono::nanoseconds>(clk::now() - st).count();
		if (elapsed_ns > limit_ns) {
			std::fprintf(stderr, "[TimeGuard] %s took %lld ns (> %lld ns)\n",
				tag, (long long)elapsed_ns, (long long)limit_ns);
			assert(false && "Time limit exceeded inside function");
		}
	}
};

#define TIME_GUARD_NS(NAME, NS) TimeGuard __tg_##__LINE__{NAME, std::chrono::nanoseconds(NS)}
#define TIME_GUARD_US(NAME, US) TimeGuard __tg_##__LINE__{NAME, std::chrono::microseconds(US)}
#define TIME_GUARD_MS(NAME, MS) TimeGuard __tg_##__LINE__{NAME, std::chrono::milliseconds(MS)}
#define TIME_GUARD_S(NAME,  S)  TimeGuard __tg_##__LINE__{NAME, std::chrono::seconds(S)}

#define TIME_GUARD(NAME, MS) TIME_GUARD_MS(NAME, MS)
...
Delaunator build_delaunay(const Polygon& poly) {
	TIME_GUARD("Delaunay", 500);
	Delaunator DTR(poly);
	return DTR;
}

void kd_init() {
	TIME_GUARD("KD-Init", 500);
	Geo::init();
}

void preprocess_wrapper(const std::vector<std::string>& data) {
	TIME_GUARD("LCP", 1500);
	preprocess(data);
}

void prim_wrapper() {
	TIME_GUARD("prim", 1000);
	prim();
}

void query() {
	TIME_GUARD("query", 2000);
	for (int q = 0; q < Q; ++q) {
		std::cin >> T;
		if (T == 1) {
			Pii s, e;
...
void solve() {
	...
	//preprocess(name);
	preprocess_wrapper(name);

	if (N < 3) { assert(0); }
	else {
		TIME_GUARD("find edges", 1000);
		bool f = 0;
		for (int i = 2; i < N; i++) {
