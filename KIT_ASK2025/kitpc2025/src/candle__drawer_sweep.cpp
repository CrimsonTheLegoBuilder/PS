	...
	for (int c = 1; c < (1 << 3); c <<= 1) {
		...
		for (int i = 0; i < sz; i++) {
			...
			for (int k = 1; k <= K; k++) {
				...
				if (!eq(se.x, se.y)) {
					grey.push_back({ L[c], w.p(se.x), w.p(se.y) });
					VP.push_back(se);
				}
			}
...
void solve() {
	...
	freopen("../tests/candle_renew/in/101.in", "r", stdin);
	freopen("../tests/candle_renew/test_101.txt", "w", stdout);
	std::cin >> N;
	P[0].resize(N);
