...
bool DIM(Pos p[], ld m, ld D) {  //diameter
	std::vector<Line> HP;
	for (int i = 0; i < N; i++) {
		ld dy = p[i + 1].y - p[i].y;
	...
	ld MD = rotating_calipers(INX);  //std::cout << MD << " " << m << "\n";
	return MD >= D;
}
ld bi_serach(Pos p[], int D) {
	ld s = 0, e = 1e7, m;
	for (int i = 0; i < 50; i++) {
		m = (s + e) / 2;
		if (DIM(p, m, D)) s = m;
