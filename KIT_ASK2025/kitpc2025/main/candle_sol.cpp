...
void query() {
	...
	for (int c = 1; c < (1 << 3); c <<= 1) {
		...
		for (int k = 1; k <= K; k++) {
			...
			if (fk) { C[0] -= c; C[k] |= c; I[c] = k; break; }
		}
		if (fk) continue;
		for (int k = 1; k <= K; k++) {//get tangents
			const Polygon& H = P[k];
			M = H.size();
			Pos pl = H[0], pr = H[0];
			for (int j = 0; j < M; j++) {
		...
		for (int k = 0; k <= K; k++) {//get visible polygon
			const Polygon& PK = P[k];
			int sz = P[k].size();
			for (int i = 0; i < sz; i++) {
				const Pos& u = PK[i], & v = PK[(i + 1) % sz];
	...
	if (!I[RED] && !I[GREEN] && !I[BLUE]) {//i & e
		for (int c = 1; c < WHITE; c++) A[c] -= A[WHITE];
	}
	for (int i = 0; i < 3; i++) {//i & e
		int c1 = (1 << ((i + 1) % 3));
		int c2 = (1 << ((i + 2) % 3));
		if (!I[c1] && !I[c2]) {
			int c = c1 | c2;
...
void solve() {
	...
	std::cin >> N; P[0].resize(N); for (Pos& p : P[0]) std::cin >> p;
	std::cin >> K;
	for (int i = 1; i <= K; i++) {
		std::cin >> M; P[i].resize(M); for (Pos& p : P[i]) std::cin >> p;
	}
	//std::cin >> Q;
	Q = 1;
	for (int q = 1; q <= Q; q++) query();
	return;
}
int main() { solve(); return 0; }//KIT ASK candle & candle & candle & shadow
