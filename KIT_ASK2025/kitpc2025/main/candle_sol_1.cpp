...
void clean(const Pos& s, const Pos& e, const Seg& s0, const int& c0, Ve& tmp, Ve& ve) {
	if (tmp.empty()) {
		if (inner_check_concave(H[c0], s0.p(.5), s, e)) {
			ve.push_back({ 0, 1 });
			ve.push_back({ 1, -1 });
		}
		return;
...
void query(const int& q) {
	...
	//std::cout << "R = [\n";
	//for (const Pos& p : H[RED]) std::cout << "  (" << p.x << ", " << p.y << "),\n";
	//std::cout << "]\n";
	//std::cout << "G = [\n";
	//for (const Pos& p : H[GREEN]) std::cout << "  (" << p.x << ", " << p.y << "),\n";
	//std::cout << "]\n";
	//std::cout << "B = [\n";
	//for (const Pos& p : H[BLUE]) std::cout << "  (" << p.x << ", " << p.y << "),\n";
	//std::cout << "]\n";
...
void solve() {
	...
	//freopen("../tests/candle/in/21.in", "r", stdin);
	//freopen("../tests/candle/ans/21.txt", "w", stdout);
	std::cin >> N;
	P[0].resize(N);
...
//		std::string Qout = Dir + "ans/" + (i < 10 ? "0" : "") + std::to_string(i) + ".out";
