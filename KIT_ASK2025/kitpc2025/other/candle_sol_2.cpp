...
Vld intersections(const Seg& l, const int& t) {
	...
	for (int i = 0; i < sz; i++) {
		...
		for (int j = 0; j < 3; j++) {
			...
			//	ret.push_back(fit(ix));
			//}
			ld ix = intersection(l, Seg(t0, t1));
			ret.push_back(fit(ix));
		}
	}
...
void solve() {
	...
	freopen("../tests/candle/in/10.in", "r", stdin);
	//freopen("../tests/candle/out_bi/56.out", "w", stdout);
	std::cin >> N;
	P[0].resize(N);
	for (Pos& p : P[0]) std::cin >> p, p *= SCALE;
	norm(P[0]);
#ifdef DEBUG
	...
	for (int i = 1; i <= K; i++) {
		std::cin >> M;
		P[i].resize(M);
		for (Pos& p : P[i]) std::cin >> p, p *= SCALE;
#ifdef DEBUG
