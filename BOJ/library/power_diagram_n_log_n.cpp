...
struct Pos3D {
	int128 x, y, z;
	int r;
	Pos3D(int128 x_ = 0, int128 y_ = 0, int128 z_ = 0, int R = 0) : x(x_), y(y_), z(z_), r(R) {}
...
Circle seed[LEN];
Vint I, PD[LEN];//power diagram (Laguerre-Voronoi diagram)
ld A;
void solve() {
	A = 0;
	int bnd = 2e4 + 10;
	...
	for (const Face& f : Hull3D) {
		if (ccw(f) < 0) {
			PD[f.v[0]].push_back(f.v[1]);
			PD[f.v[0]].push_back(f.v[2]);
			PD[f.v[1]].push_back(f.v[0]);
			PD[f.v[1]].push_back(f.v[2]);
			PD[f.v[2]].push_back(f.v[0]);
			PD[f.v[2]].push_back(f.v[1]);
			I.push_back(f.v[0]);
			I.push_back(f.v[1]);
			I.push_back(f.v[2]);
		}
	...
	for (const int s : I) {
		...
		if (!ra) continue;
		std::sort(PD[s].begin(), PD[s].end());
		PD[s].erase(unique(PD[s].begin(), PD[s].end()), PD[s].end());
		Vlinear HP;
...
int main() {
	...
	while (Q--) {
		I.clear();
		for (int i = 0; i < 3001; i++) PD[i].clear();
		solve();
	}
	return 0;
}