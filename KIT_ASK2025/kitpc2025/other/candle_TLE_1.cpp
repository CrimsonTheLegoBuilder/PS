...
void query(const int& q) {
	...
	for (int c = 1; c < (1 << 3); c <<= 1) {
		...
		for (int z = 0; z < sz; z++) {
			int z1 = z;
			H[c].push_back(Z[c][z1].s);
			H[c].push_back(Z[c][z1].e);
		}
		...
		//	int z1 = z;
		//	int z2 = (z + 1) % sz;
		//	if (Z[c][z0].e != Z[c][z1].s) H[c].push_back(Z[c][z1].s);
		//	if (Z[c][z1].e != Z[c][z2].s) H[c].push_back(Z[c][z1].e);
		//}
		//std::cout << "ttt\n";
		//for (auto tt : T[c]) {
		//	std::cout << tt[1] << "\n";
	...
	for (int i = 0; i < 3; i++) {//get intersections
		int c1 = (1 << ((i + 1) % 3));
		int c2 = (1 << ((i + 2) % 3));
		if (!I[c1] && !I[c2]) {
			for (int _ = 0; _ < 2; _++) {
	...
	if (!I[RED] && !I[GREEN] && !I[BLUE]) {//R & G & B
		...
		for (const Seg& se : VS) {
			...
			for (int i = 0; i < 3; i++) {
				int c = 1 << i;
				if (c == se.i) continue;
				const Pos& s = se.s, & e = se.e;
				Pos m = se.p(.5);
				if (!inner_check_concave(H[c], m, s, e)) {
					f = 0;
					break;
				}
			}
...
void solve() {
	...
	freopen("../tests/candle/in/101.in", "r", stdin);
	//freopen("../tests/candle/tle1/67.txt", "w", stdout);
	std::cin >> N;
	P[0].resize(N);
