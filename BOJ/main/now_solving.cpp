...
inline bool zero(ll x) { return !x; }
...
struct Pos {
	...
	Pos operator - (const Pos& p) const { return { x - p.x, y - p.y }; }
	Pos operator * (int n) const { return { x * n, y * n }; }
	Pos operator / (int n) const { return { x / n, y / n }; }
...
// ---- dynamic-hull onion peeling: static balanced tree over x-sorted P, deletion-only ----
namespace OP {
	std::vector<int> lc, rc, par;
	std::vector<std::vector<int>> up, lo;   // per-node upper / lower hull (indices into sorted P)
	std::vector<int> leafOf;                 // sorted point index -> leaf node id
	int root;
	inline ll tcross(const Pos& o, const Pos& a, const Pos& b) {
		return (a.x - o.x) * (b.y - o.y) - (a.y - o.y) * (b.x - o.x);
	}
	std::vector<int> merge_upper(const std::vector<int>& A, const std::vector<int>& B) {
		if (A.empty()) return B;
		if (B.empty()) return A;
		int i = (int)A.size() - 1, j = 0;
		for (;;) {
			if (j + 1 < (int)B.size() && tcross(P[A[i]], P[B[j]], P[B[j + 1]]) > 0) { j++; continue; }
			if (i - 1 >= 0 && tcross(P[B[j]], P[A[i]], P[A[i - 1]]) < 0) { i--; continue; }
			break;
		}
		std::vector<int> h; h.reserve(i + 1 + (int)B.size() - j);
		for (int k = 0; k <= i; k++) h.push_back(A[k]);
		for (int k = j; k < (int)B.size(); k++) h.push_back(B[k]);
		return h;
	}
	std::vector<int> merge_lower(const std::vector<int>& A, const std::vector<int>& B) {
		if (A.empty()) return B;
		if (B.empty()) return A;
		int i = (int)A.size() - 1, j = 0;
		for (;;) {
	...
	int build(int a, int b, int p) {
		int id = lc.size();
		lc.push_back(-1); rc.push_back(-1); par.push_back(p);
		up.emplace_back(); lo.emplace_back();
		if (b - a == 1) { up[id] = { a }; lo[id] = { a }; leafOf[a] = id; return id; }
		int mid = (a + b) / 2;
		int Lc = build(a, mid, id), Rc = build(mid, b, id);
		lc[id] = Lc; rc[id] = Rc;
		up[id] = merge_upper(up[Lc], up[Rc]);
		lo[id] = merge_lower(lo[Lc], lo[Rc]);
		return id;
	}
	void init(int n) {
		lc.clear(); rc.clear(); par.clear(); up.clear(); lo.clear();
		lc.reserve(2 * n); rc.reserve(2 * n); par.reserve(2 * n); up.reserve(2 * n); lo.reserve(2 * n);
		leafOf.assign(n, -1);
		root = build(0, n, -1);
	}
	void kill(int pi) {                      // delete point (sorted index pi), update ancestors
		int v = leafOf[pi];
		up[v].clear(); lo[v].clear();
		for (v = par[v]; v != -1; v = par[v]) {
			up[v] = merge_upper(up[lc[v]], up[rc[v]]);
			lo[v] = merge_lower(lo[lc[v]], lo[rc[v]]);
		}
	}
}
struct Event {
...
struct Jaw {
	...
	bool valid(const Pos& cur, const Pos& vec, bool evt = 1) {
	...
	void init(const Polygon& Q, int N, int K_, int h_, const Pos& cur = Pos(0, -1)) {
		K = K_; h = h_;
	...
	bool candidate_update(const Pos& p, const Pos& cur, int idx[], int f = HEAD, int o = -1, int pop_ = 0) {
		int i = 0;
		int& c = (f == HEAD ? hp : tp);
		if (o != -1) {
			int x = idx[o];
		...
		{
			int lo = 0, hi = c;
			while (lo < hi) { int mid = (lo + hi) >> 1; if (valid(cur, P[idx[mid]] - p, 0)) hi = mid; else lo = mid + 1; }
			i = lo;
		}
		if (i < c) {
	...
	void swap(int i, int j, int idx[], int f) {
...
struct Calipers {
	...
	void init(int n = -1, int k = -1, Pos c = Pos(0, -1)) {
		...
		OP::init(N);
		for (int k = 0; k <= K; k++) {
			if (OP::up[OP::root].empty()) break;
			h++;
			// H[k] = lo[root] + reverse(up[root])[1 .. len-2]  (== monotone_chain order)
			H[k].clear();
			for (int id : OP::lo[OP::root]) H[k].push_back(P[id]);
			for (int t = (int)OP::up[OP::root].size() - 2; t >= 1; t--) H[k].push_back(P[OP::up[OP::root][t]]);
			int sz = H[k].size();
			...
			for (const Pos& p : H[k]) { F[p.pi] = 1; OP::kill(p.pi); }
		}
		for (int k = 0; k < h; k++) {
	...
	bool jaw_rotate() {
		Pos V[6]; int sz = 0;
		get_events();
		if (bot.EQ.size()) V[sz++] = bot.EQ.top().v();
		if (bot.JQ.size()) V[sz++] = bot.JQ.top().v();
		if (bot.SQ.size()) V[sz++] = bot.SQ.top().v();
		if (top.EQ.size()) V[sz++] = -top.EQ.top().v();
		if (top.JQ.size()) V[sz++] = -top.JQ.top().v();
		if (top.SQ.size()) V[sz++] = -top.SQ.top().v();
		if (!sz) { cur = Pos(0, 1); return 0; }
		cur = V[0];
