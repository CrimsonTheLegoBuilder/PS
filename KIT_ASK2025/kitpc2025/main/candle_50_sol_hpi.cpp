...
struct Seg {
	Pos s, e, dir;
	Seg(Pos s_ = Pos(), Pos e_ = Pos()) : s(s_), e(e_) { dir = (e - s).unit(); }
...
//Polygon intersection(const Polygon& a, const Polygon& b) { return sutherland_hodgman(a, b); }
...
//	return ret;
//}
Polygon half_plane_intersection(Segs& HP, const bool& srt = 1) {
	...
	if (sz < 3) return {};
	Polygon HPI;
	for (int i = 0; i < sz; ++i) HPI.push_back(intersection_(dq[i], dq[(i + 1) % sz]));
	return graham_scan(HPI);
}
struct Vec {
	ld vy, vx;
...
Line line(const Pos& s, const Pos& e) {
	ld dy, dx, c;
	dy = e.y - s.y;
	dx = s.x - e.x;
	c = dy * s.x + dx * s.y;
...
Line line(const Vec& s, const Pos& p) {
...
Polygon intersection(const Polygon& a, const Polygon& b, const Polygon& c = {}) {
	...
	sz = a.size();
	for (int i = 0; i < sz; i++) {
		if (a[i] == a[(i + 1) % sz]) continue;
		vs.push_back(line(a[i], a[(i + 1) % sz]));
	}
	sz = b.size();
	for (int i = 0; i < sz; i++) {
		if (b[i] == b[(i + 1) % sz]) continue;
		vs.push_back(line(b[i], b[(i + 1) % sz]));
	}
	sz = c.size();
	for (int i = 0; i < sz; i++) {
		if (c[i] == c[(i + 1) % sz]) continue;
		vs.push_back(line(c[i], c[(i + 1) % sz]));
	}
	Polygon hpi;
	bool f = half_plane_intersection(vs, hpi);
	return hpi;
}
...
int main() {
	...
	//freopen("../tests/candle_renew/in/94.in", "r", stdin);
	//freopen("../tests/candle_renew/out_hpi/94.out", "w", stdout);
	solve();
	return 0;
...
//		std::string Qout = Dir + "out_hpi/" + (i < 10 ? "0" : "") + std::to_string(i) + ".out";
