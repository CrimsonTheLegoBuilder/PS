...
struct Event {
	ld x;
	int f, i;
	Event(ld x_ = 0, int f_ = 0, int i_ = -1) : x(x_), f(f_), i(i_) {}
	bool operator < (const Event& e) const { return (eq(x, e.x)) ? f > e.f : x < e.x; }
};
...
inline void solve() {
	...
	for (int i = 0; i < N; i++) {
		...
		for (int j = 0; j < N; j++) {
			...
			if (vec.Euc() <= sq(ra - rb)) {
				if (ra >= rb) continue;
				ve.push_back(Event(0, 1, j));
				ve.push_back(Event(2 * PI, -1, j));
				continue;
			}
			Polygon IX = intersection(VC[i], VC[j]);
			if (IX.empty()) continue;
			Pos inx = IX[0];
			ld lo = inx.x, hi = inx.y;
			if (lo < hi) {
				ve.push_back(Event(lo, 1, j));
				ve.push_back(Event(hi, -1, j));
			}
			else {
				ve.push_back(Event(lo, 1, j));
				ve.push_back(Event(2 * PI, -1, j));
		...
		int cnt = 0;
		for (int j = 0; j < sz; j++) {
			if (j < sz - 1 && !eq(ve[j].x, ve[j + 1].x) || j == sz - 1) {

			}
			if (ve[j].f > 0) { I.insert(j); cnt += 1; }
			else { I.erase(j); cnt -= 1; }
		}
	}
