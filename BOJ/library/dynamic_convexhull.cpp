...
ll area(const std::vector<Pos>& H, int lo = 1) {
	ll ret = 0;
	int sz = H.size();
	for (int i = 0; i < sz; i++) {
...
private:
	...
	bool exist(const Pos& p) { return hull.count(transform(p)); }

	int len(const Pos& p) { return search(transform(p).x); }
