...
Polygon intersection(const Polygon& a, const Polygon& b, const Polygon& c = {}) {
	...
	int sz;
	sz = a.size();
	for (int i = 0; i < sz; i++) vs.push_back(Seg(a[i], a[(i + 1) % 3]));
	sz = b.size();
	for (int i = 0; i < sz; i++) vs.push_back(Seg(b[i], b[(i + 1) % 3]));
	sz = c.size();
	for (int i = 0; i < sz; i++) vs.push_back(Seg(c[i], c[(i + 1) % 3]));
	return half_plane_intersection(vs);
}
...
int main() {
	...
	//freopen("../tests/candle/in/24.in", "r", stdin);
