...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i < 4; i++) rnd.next(1, 1000);
	std::cout << "generating start\n";
	Vpos C, PR;
	int cnt = 0;
	Pos s = Pos(rnd.next(1, 1000), 0);
	int B = 16;//sum of inner polygon
	int sum_b = 50;//sum of virtice of inner polygon
	int v = sum_b / B;
	while (C.size() < B * 10) {
		cnt = (cnt + 1) % 100;
	...
	freopen("../tests/candle_renew/in/79.in", "w", stdout);
	std::cout << P[0].size() << "\n";
